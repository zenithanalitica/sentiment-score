...
from transformers import (
    ...
    XLMRobertaTokenizer,  # pyright: ignore[reportPrivateImportUsage]
)
...
def load_sentiment_model_and_tokenizer(model_name: str, device: torch.device):  # pyright: ignore[reportUnknownParameterType]
    """
    ...
    try:
        tokenizer = XLMRobertaTokenizer.from_pretrained(model_name, use_fast=True)  # pyright: ignore[reportUnknownVariableType, reportUnknownMemberType]
        ...
        tokenizer = XLMRobertaTokenizer.from_pretrained(model_name, use_fast=False)  # pyright: ignore[reportUnknownVariableType, reportUnknownMemberType]
        print("Loaded slow XLMRobertaTokenizer.")

    # To get label mapping (id2label)
    config = AutoConfig.from_pretrained(  # pyright: ignore[reportUnknownVariableType, reportUnknownMemberType]
        model_name
    )
    ...
    model.to(  # pyright: ignore[reportUnknownMemberType]
        device
    )
    ...
    return tokenizer, model, config  # pyright: ignore[reportUnknownVariableType]


# --- Neo4j Bolt Setup ---
def connect_to_neo4j(bolt_url: str, user: str, password: str) -> neo4j.Driver:
    """
...
def load_progress(filename: str) -> int:
    ...
    if os.path.exists(filename):
        try:
            ...
            print(
                f"Warning: Could not read progress file {filename}. Starting from 0. Error: {e}"
            )
            return 0
...
def get_total_unprocessed_tweets_tx_fn(tx: neo4j.ManagedTransaction) -> int:
...
def fetch_tweets_batch_tx_fn(
    tx: neo4j.ManagedTransaction, skip: int, limit: int
...
def update_tweets_batch_tx_fn(
    tx: neo4j.ManagedTransaction, batch_data: list[dict[str, float | int]]
):
    ...
    """
    _ = tx.run(query, batch_data=batch_data)


# --- Main Logic Functions ---
def get_total_unprocessed_tweets_count(driver: neo4j.Driver) -> int:
    ...
    try:
        ...
        with driver.session() as session:
            total_tweets_to_process = session.execute_read(
                get_total_unprocessed_tweets_tx_fn
            )
        print(f"Total tweets matching criteria: {total_tweets_to_process}")
        return total_tweets_to_process
    except neo4j.exceptions.Neo4jError as e:  # pyright: ignore[reportAny]
...
def fetch_tweets_batch(
    driver: neo4j.Driver, skip: int, limit: int
    ...
    try:
        ...
        with driver.session() as session:
            fetched_tweets_data = session.execute_read(
                fetch_tweets_batch_tx_fn, skip, limit
            )
...
def process_batch_for_sentiment(
    fetched_tweets_data: list[dict[str, Any]],  # pyright: ignore[reportExplicitAny]
    ...
    texts_batch = [preprocess(tweet["text"]) for tweet in fetched_tweets_data]  # pyright: ignore[reportAny]

    all_update_params: list[dict[str, float | int]] = []
    total_texts = len(texts_batch)

    # Process in smaller inference batches
    for i in tqdm(
        range(0, total_texts, inference_batch_size),
        desc=f"Inferring {total_texts} tweets",
    ):
        ...
        with torch.no_grad():
            outputs = model(**inputs)  # pyright: ignore[reportCallIssue, reportUnknownVariableType]

        # Get probabilities and calculate sentiment scores/labels
        probabilities = torch.softmax(outputs.logits, dim=1).cpu().numpy()  # pyright: ignore[reportUnknownVariableType, reportUnknownMemberType, reportUnknownArgumentType]

        for j in range(probabilities.shape[0]):  # pyright: ignore[reportUnknownMemberType, reportUnknownArgumentType]
            scores: list[float] = probabilities[j]  # pyright: ignore[reportAny]
            ...
            sentiment_label_val = config.id2label[predicted_class_id]  # pyright: ignore[reportAttributeAccessIssue, reportUnknownMemberType, reportUnknownVariableType]
...
def update_neo4j_with_sentiment(
    driver: neo4j.Driver, update_params: list[dict[str, float | int]], current_skip: int
) -> bool:
    """
    ...
    try:
        ...
        with driver.session() as session:
            session.execute_write(update_tweets_batch_tx_fn, batch_data=update_params)
        print(
            f"Successfully updated {len(update_params)} tweets in Neo4j for batch starting at {current_skip}."
        )
        return True
    except neo4j.exceptions.Neo4jError as e:  # pyright: ignore[reportAny]
...
def main():
    ...
    tokenizer, model, config = load_sentiment_model_and_tokenizer(MODEL_NAME, device)  # pyright: ignore[reportUnknownVariableType]
    ...
    if total_tweets_to_process > 0 and skip > total_tweets_to_process:
        print(
            f"Warning: Loaded skip value ({skip}) is greater than current total ({total_tweets_to_process}). Resetting skip to total."
        )
    ...
    while True:
        ...
        if not fetched_tweets_data:
            print(
                f"No more tweets matching criteria found in batch starting at skip {skip}. Assuming processing is complete."
            )
            break
        ...
        all_update_params = process_batch_for_sentiment(
            fetched_tweets_data,
            tokenizer,  # pyright: ignore[reportUnknownArgumentType]
        ...
        if update_successful:
            ...
            print(
                f"Progress: {processed_count} / {total_tweets_to_process} tweets processed."
            )
    ...
    print(
        f"Estimated processed based on last saved skip: {reported_processed} / {total_tweets_to_process}"
    )
