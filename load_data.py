...
# ————— Neo4j HTTP setup —————
auth = HTTPBasicAuth("neo4j", credentials.password)
...
while True:
    ...
    response = requests.post(credentials.url, json=batch_query, auth=auth)
    data = response.json()
    tweets = [
    ...
    for tweet in tqdm(
        ...
        update_response = requests.post(credentials.url, json=update_query, auth=auth)
